...
void init_button(Button *button, uint8_t *port, uint8_t number, uint8_t click_ev, uint8_t hold_ev, void (*callback)(uint8_t))
{
	button->pin = port;
	button->number = number;
	button->hold_timer = 0;
	button->click_event = click_ev;
	button->hold_event = hold_ev;
	button->callback = callback;
	button->state = BT_FREE;
}
